package com.sipa.boot.java8.common.common.jpush.component;
...
public class JPushApi {
        ...
        if (jPushProperties.isUseProxy()) {
            ...
            if (StringUtils.isNotEmpty(jPushProperties.getProxyUsername())) {
                proxy = new HttpProxy(StringUtils.trim(jPushProperties.getProxyHost()), jPushProperties.getProxyPort(),
                    StringUtils.trim(jPushProperties.getProxyUsername()),
                    StringUtils.trim(jPushProperties.getProxyPassword()));
            } else {
                proxy = new HttpProxy(StringUtils.trim(jPushProperties.getProxyHost()), jPushProperties.getProxyPort());
            }
        }
        jPushClient =
            new JPushClient(jPushProperties.getMasterSecret(), jPushProperties.getAppKey(), proxy, clientConfig);
        initSuccess = true;
    ...
    private Long executePush(PushPayload payload) {
        ...
        try {
            ...
            logger.error("推送消息出现异常, status={}, errorCode={}, errorMessage={}", e.getStatus(), e.getErrorCode(),
                e.getErrorMessage());
        }
        return null;
    }
