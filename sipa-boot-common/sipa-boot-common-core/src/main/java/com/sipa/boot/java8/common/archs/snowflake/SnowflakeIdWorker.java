...
public class SnowflakeIdWorker {
    // 开始时间截 (2020-03-01)
    private static final long START_TIME = 1582992000L;

    // uid的位数
    private static final long ID_BITS = 64L;
    ...
    // 机器id所占的位数
    private static final long WORKER_ID_BITS = 9L;

    // 序列在id中占的位数
    // 单机支持4096TPS（每次序列号随到0）
    private static final long SEQUENCE_BITS = ID_BITS - HIGH_BITS - TS_BITS - DATA_CENTER_ID_BITS - WORKER_ID_BITS;

    // 支持的最大数据中心id，结果是1
    private static final long MAX_DATA_CENTER = ~(-1L << DATA_CENTER_ID_BITS);

    // 支持的最大机器id，结果是511
    private static final long MAX_WORKER = ~(-1L << WORKER_ID_BITS);
    ...
    public SnowflakeIdWorker(long dataCenterId) {
        this(dataCenterId, getIpCode(), 5L);
    }
     ...
     *            工作ID (0~510(255+255))
     */
    public SnowflakeIdWorker(long dataCenterId, long workerId, long timeOffset) {
     ...
     * 用ip地址最后几个字节标示.
     * <p>
     * ip网段需要是 192.168.0.0 - 192.168.255.255 或者 172.x.0.0 - 172.x.255.255 段，10段不可以！！！！！！
     *
     * @return worker id
     */
    public static int getIpCode() {
        if (IP_CODE == 0) {
            try {
                ...
                IP_CODE = (0x000000FF & lastSecondIp) + (0x000000FF & lastIp);
            } catch (Exception e) {
