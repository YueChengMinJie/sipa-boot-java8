 ...
 * @date 2021/10/2
 */
public abstract class AbstractLocalCache<K, V> implements IClusterCache<K, V> {
    private final Cache<K, Object> cache;

    private final IClusterCache<K, V> clusterCache;

    private final boolean cacheEmpty;

    public AbstractLocalCache(IClusterCache<K, V> clusterCache, Cache<K, Object> localCache, boolean cacheEmpty) {
        this.cache = localCache;
        this.clusterCache = clusterCache;
        this.cacheEmpty = cacheEmpty;
    }

    public AbstractLocalCache(IClusterCache<K, V> clusterCache, Cache<K, Object> localCache) {
    ...
    public Flux<Map.Entry<K, V>> get(Collection<K> key) {
        ...
        return clusterCache.get(key).doOnNext(kvEntry -> {
            K k = kvEntry.getKey();
    ...
    public Mono<Boolean> put(K key, V value) {
        ...
        return Mono.defer(() -> {
            cache.put(key, value);
            return clusterCache.put(key, value).flatMap(r -> onUpdate(key, value)).thenReturn(true);
        });
    }
    ...
    public Mono<Boolean> putIfAbsent(K key, V value) {
        ...
        return Mono.defer(() -> {
            cache.invalidate(key);
            return clusterCache.putIfAbsent(key, value).flatMap(r -> onUpdate(key, value).thenReturn(r));
        });
    }
    ...
    public Mono<Boolean> remove(K key) {
        ...
        return Mono.defer(() -> {
            cache.invalidate(key);
            return clusterCache.remove(key).flatMap(r -> onRemove(key)).thenReturn(true);
        });
    }
    ...
    public Mono<V> getAndRemove(K key) {
        ...
        return Mono.defer(() -> {
            cache.invalidate(key);
            return clusterCache.getAndRemove(key).flatMap(r -> onRemove(key).thenReturn(r));
        });
    }
    ...
    public Mono<Boolean> putAll(Map<? extends K, ? extends V> multi) {
        ...
        return Mono.defer(() -> {
            ...
            return clusterCache.putAll(multi).then(onRemove(multi.keySet())).thenReturn(true);
        });
    }
    ...
    public Mono<Void> clear() {
        return Mono.defer(() -> {
            cache.invalidateAll();
            return clusterCache.clear().then(onClear());
        });
    }
